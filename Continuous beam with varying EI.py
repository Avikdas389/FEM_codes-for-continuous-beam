# -*- coding: utf-8 -*-
"""
Created on Tue Mar 26 09:07:56 2024
...
for i in range(N_element):
    n = i
    f = n+1
    K[2*n,2*n]=K[2*n,2*n]+12*EI[i]/(l[i]**3)
    K[2*n,2*n+1]=K[2*n,2*n+1]+6*EI[i]/(l[i]**2)
    K[2*n,2*f]=K[2*n,2*f]-12*EI[i]/(l[i]**3)
    K[2*n,2*f+1]=K[2*n,2*f+1]+6*EI[i]/(l[i]**2)
    
    K[2*n+1,2*n]=K[2*n+1,2*n]+6*EI[i]/(l[i]**2)
    K[2*n+1,2*n+1]=K[2*n+1,2*n+1]+4*EI[i]/l[i]
    K[2*n+1,2*f]=K[2*n+1,2*f]-6*EI[i]/(l[i]**2)
    K[2*n+1,2*f+1]=K[2*n+1,2*f+1]+2*EI[i]/l[i]
    
    K[2*f,2*n]=K[2*f,2*n]-12*EI[i]/(l[i]**3)
    K[2*f,2*n+1]=K[2*f,2*n+1]-6*EI[i]/(l[i]**2)
    K[2*f,2*f]=K[2*f,2*f]+12*EI[i]/(l[i]**3)
    K[2*f,2*f+1]=K[2*f,2*f+1]-6*EI[i]/(l[i]**2)
    
    K[2*f+1,2*n]=K[2*f+1,2*n]+6*EI[i]/(l[i]**2)
    K[2*f+1,2*n+1]=K[2*f+1,2*n+1]+2*EI[i]/l[i]
    K[2*f+1,2*f]=K[2*f+1,2*f]-6*EI[i]/(l[i]**2)
    K[2*f+1,2*f+1]=K[2*f+1,2*f+1]+4*EI[i]/l[i]
...
for i in range(0,N_element):
  for j in range(i*2,i*2+2):
     if j % 2 == 0:
      M_final[j]= M_fend[j] + 4*EI[i]*All_DOF[j+1]/l[i] + 2*EI[i]*All_DOF[j+3]/l[i] -6*EI[i]*(All_DOF[j+2]-All_DOF[j])/l[i]**2
     else:
      M_final[j]= M_fend[j] + 4*EI[i]*All_DOF[j+2]/l[i] + 2*All_DOF[j]*EI[i]/l[i] -6*EI[i]*(All_DOF[j+1]-All_DOF[j-1])/l[i]**2  
...
for i in range(0,N_element):
    ...
    for j in range(N_div):
        P_z[j] = P[N_div*i + j]        
    plt.plot(x_values,P_z, marker='o',linestyle='-', color = 'red',label='Moment due to loading')
